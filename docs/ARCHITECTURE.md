...
- **Custom theme colours use arb-value CSS-var syntax, not named utilities.** When a component needs one of the package's own palette colours (`brand`, or any future custom palette), reference it as `bg-(--color-brand-600)`, `border-(--color-brand-600)`, `text-(--color-brand-600)` — Tailwind v4's arbitrary-value form — rather than `bg-brand-600`. Reason: the named utility requires the consumer's Tailwind to register the package's `@theme inline { --color-brand-N: var(--color-brand-N); }` block, which is fragile downstream (we observed it failing in `overseer-local` even with the canonical `@import "tailwindcss";` entry and the package path added to `content`). The arb-value form is robust because the CSS variable is always defined by the package's `tokens.css`, and the resulting `.bg-\(--color-brand-600\){background-color:var(--color-brand-600)}` utility doesn't depend on the consumer's theme setup. Tailwind v4's **default-palette** colours (emerald, teal, purple, …) are exempt — they still work as named utilities (`bg-emerald-500`) because they don't go through the `@theme` registration.
