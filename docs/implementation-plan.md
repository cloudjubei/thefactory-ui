...
#### Sign-off + launch surfaces — deferred half

The sign-off panel and the start-work dock ship against the facts the backend files today. Each item below is blocked on a backend capability that does not exist yet, NOT on UI work; the UI deliberately says nothing rather than guessing, so none of these is a visible gap.

- **Pixel-diff classification of a screen pair.** `ScreenPair.class` is `pair | new | removed | single` — derived from which phases were filed, never from comparing pixels. There is no changed/unchanged claim anywhere and the overlay offers Mirror/Slide only, because a Diff mode with nothing to diff would be a lie. Trigger: a backend that computes and files a per-pair pixel delta. Then `class` gains changed/unchanged, the Screens strip can sort changed-first, and Diff joins the overlay's mode switch.
- **Beat script for the walkthrough.** The Walkthrough tab plays the recording with the platform's own controls. A scrubber tied to labelled beats needs the agent to file a beat list (timestamp + what it is doing) alongside the video. Trigger: a `beats` field on filed recording evidence.
- **Per-check "run now".** A `run`-fillable method runs the project's whole verification pass. Running ONE check needs a backend route that takes a check id. Trigger: `POST /cli-runs/:id/verify` accepting a check filter.
- **Extra approve variants** — merge-when-green, rewrite the commit message, merge-as. `earnedApproveActions` already models a primary + a rest menu, so each is one descriptor plus its executor. Trigger: the corresponding `approveCliRunReview` actions.
- **Runner override at decide time.** `decisionOverrides` deliberately whitelists `proofRequired` and `note` only — a decision must never change WHAT runs. Letting the approver switch API/CLI at the approval means widening that whitelist, which needs its own think about what an approval is allowed to be. Trigger: an explicit decision that the approver may pick the executor.
- **Launch options on the API transport.** The dock's note + proof toggle reach the run through `decisionMetadata`, which only a CLI-brokered decision carries — the API path confirms tool calls by ID alone (`confirmTools(ctx, grantedToolCallIds)`), so the agent's own arguments are what runs. The dock now shows those choices read-only on an API chat rather than collecting input it would silently drop (`launchOptionsAreHonoured`). Trigger: a per-tool-call argument-override channel on the API confirm path (backend route + the agent loop applying it); then drop the gate and both transports behave alike.
- **Native dependency-bullet navigation.** Web's `DependencyBullet` is a connected component that routes to the story; the native peer is presentational and needs an `onPress` the launch dock has no seam to supply, so its bullets are `interactive={false}` rather than tappable-and-dead. Trigger: a navigation callback threaded to the native `ApprovalPanel` (mobile already has `/projects/[projectId]/stories/[storyId]`).
- **Mobile `onSaveFile` host implementation.** The native panel takes an `onSaveFile` host slot and renders the save affordances (screen pair → two PNGs, report → markdown) only when the app provides it — the same shape as `onOpenGit`, because writing a file where a person can find it needs a platform capability this package must not depend on. overseer-mobile has `expo-file-system` but no share sheet, so it passes nothing today and the buttons stay hidden. Trigger: the mobile app adopting `expo-sharing` (or a media-library write); then it implements the handler and both clients can save. Native recordings stay unsavable for a different reason — `WalkthroughTab` deliberately never fetches their bytes.
- **Motion evidence (gif / short clip) for a screen pair.** Asked for during design: a set that is animated should say so with its own icon. Needs the agent to file a motion artefact against a pair's subject. Trigger: a recording filed with a pair's `subject` rather than standalone.
