...
describe('interpolatePrompt', () => {
  ...
  it('serialises group_projects as a JSON array, defaulting to []', () => {
    ...
    expect(interpolatePrompt('{{group_projects}}', { group: { projects: [{ id: 'p1' }] } })).toBe(
      '[{"id":"p1"}]',
    )
  })

  it('replaces every occurrence of a repeated placeholder', () => {
    expect(interpolatePrompt('{{project_id}}-{{project_id}}', { project: { id: 'x' } })).toBe('x-x')
  })

  it('replaces the agent run type placeholder', () => {
    expect(interpolatePrompt('{{agent_run_type}}', { agentRunType: 'developer' })).toBe('developer')
  })
})
...
describe('buildChatPromptVariables', () => {
  ...
  const getStory = (id: string) =>
    id === 's1'
      ? { id: 's1', title: 'Auth', description: 'Login', features: [{ id: 'f1' }] }
      : undefined
