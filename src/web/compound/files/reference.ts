 ...
 *
 * The shape mirrors `MentionParse`/`applyMention` exactly so the textarea can
 * thread both flows through the same caret-detection / accept-suggestion
 * machinery. Library is decoupled from any reference source — the consumer
 * supplies suggestions via `onSearchReferences`.
 */
