...
export default function ReviewTabBar({ tabs, active, onChange }: ReviewTabBarProps) {
  return (
    <div
      role="tablist"
      className="flex gap-0.5 overflow-x-auto overflow-y-hidden shadow-[inset_0_-1px_0_var(--border-subtle)] [scrollbar-width:none]"
    >
      {tabs.map((tab) => {
        ...
        return (
          ...
          >
            ...
            {tab.count !== undefined ? (
              <span
                className={`rounded-full border px-1.5 py-0.5 text-[9.5px] font-semibold leading-none tabular-nums ${
                  selected
